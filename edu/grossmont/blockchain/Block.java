...
public class Block {
    ...
    private synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        ...
        for(int i = 0; i < lstItems.size(); i++ ){
            copy.add(lstItems.get(i));
        }


        for(int i = 0; i < copy.size(); i++ ){

            copy.set(i, BlockchainUtil.generateHash(copy.get(i)));
        ...
        while (copy.size() > 1) {
            ...
            for(int i = 0; i < copy.size(); i = i + 2 ){
                ...
                String sItem1 = copy.get(i);
                String sItem2 = copy.get(i + 1);

                sTemp.add(BlockchainUtil.generateHash(sItem1 + sItem2));


            }

            copy.clear();
            for (int x = 0; x < sTemp.size(); x++){

                copy.add(sTemp.get(x));
